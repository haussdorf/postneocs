...
function display() {
  ...
  const charNameDisp = char => {
    const charName = reduceTextWidth(char.name, 'Arial 12.8px', 220);
    const charTooltip = char.name;
    // const charTooltip = char.tooltip ? char.tooltip : '';
    return `<p title="${charTooltip}">${charName}</p>`;
  };

  progressBar(`Выборы #${battleNo}`, percent);

  document.querySelector('.left.sort.image').src = leftChar.img;
  document.querySelector('.left.sort.image').title = leftChar.tooltip;
  document.querySelector('.right.sort.image').src = rightChar.img;
  document.querySelector('.right.sort.image').title = rightChar.tooltip;
...
function pick(sortType) {
  ...
  if (leftIndex < 0) {
    ...
    progressBar(`Выборы #${battleNo} - GOLOSOVANIE ZAVERSHENO`, 100);
...
function result(imageNum = 10) {
  ...
  const timeStr = `Сейчас уже [${new Date(timestamp + timeTaken).toLocaleString('ru-RU')}] и ты просрал [${msToReadableTime(timeTaken)}] своего времени в постнина сортере, надеюсь тебе было весело.<br><br><a href="${location.protocol}//${sorterURL}">Давай по новой, все хуйня</a>`;
...
function saveProgress(saveType) {
  ...
  if (saveType !== 'Autosave') {
    ...
    const inProgressText = 'Ну вот сейв хуй знает.';
    const finishedText = 'Вот твоя ссылка для расшаривания мнения.';
...
function generateImage() {
  ...
  html2canvas(document.querySelector('.results')).then(canvas => {
    ...
    imgButton.innerHTML = '';
    imgButton.insertAdjacentHTML('beforeend', `<a href="${dataURL}" download="${filename}">Скачать</a><br><br>`);

    resetButton.insertAdjacentText('beforeend', 'Удалить');
    resetButton.addEventListener('click', (event) => {
      imgButton.addEventListener('click', generateImage);
      imgButton.innerHTML = 'Сделать картинку';
...
function msToReadableTime (milliseconds) {
  ...
  t = t - (minutes * 60);
  const content = [];
	if (years) content.push(years + " годиков");
	if (months) content.push(months + " месяцев");
	if (days) content.push(days + " дней");
	if (hours) content.push(hours + " часов");
	if (minutes) content.push(minutes + " минут");
	if (t) content.push(t + " секунд");
